...
export class rentACar1651407490529 implements MigrationInterface {
    name = 'rentACar1651407490529'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`vehicle\` (\`id\` int NOT NULL AUTO_INCREMENT, \`type\` enum ('auto', 'vizijármű') NOT NULL DEFAULT 'auto', \`brand\` varchar(255) NOT NULL, \`brand_type\` varchar(255) NOT NULL, \`status\` enum ('szabad', 'kikölcsönzött', 'selejtezett') NOT NULL DEFAULT 'szabad', \`km\` int NOT NULL, \`license_plate_number\` varchar(255) NOT NULL, \`procurement_date\` datetime NOT NULL, \`daily_price\` int NOT NULL, \`rent_price\` int NOT NULL, \`km_price\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`rent\` (\`id\` int NOT NULL AUTO_INCREMENT, \`rented_at\` datetime NOT NULL, \`rented_end\` datetime NULL, \`is_damaged\` tinyint NULL, \`status\` enum ('folyamatban', 'befejezett') NOT NULL DEFAULT 'folyamatban', \`km\` int NULL, \`final_price\` int NOT NULL, \`client_id\` int NOT NULL, \`vehicle_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
